...
class LoadLocalEntityPos(CommandBase):
    ...
    def get_commands(self):
        return [
            f"execute store result s{self.index_stack} {add_suffix_function(self.function, '_fSk')} data"
            f" get entity {get_selector_for_entityref(LocalEntity(self.i), self.function)} Pos{self.p_var}"
        ]
